...
const MOVEMENT_TYPES = ['إدخال', 'إخراج', 'تحويل'];

const MOVEMENT_DATA = {
  id: '#WM-20260406-00006',
  warehouse: 'المستودع الرئيسي',
  type: 'إدخال',
  notes: '',
  items: [
    { id: 1, name: 'قماش قطني أبيض', qty: 100 },
    { id: 2, name: 'خيط بوليستر',    qty: 50  },
  ],
};

interface ProductItem { id: number; name: string; qty: number; }

export default function EditWarehouseMovementPage() {
  const navigate = useNavigate();
  const [items, setItems] = useState<ProductItem[]>(MOVEMENT_DATA.items);
    ...
    <div className="flex flex-col gap-6 h-full">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-400 dark:text-white/30 font-bold">
          ...
          <span className="text-slate-700 dark:text-white/80">تعديل حركة مخزن</span><span>/</span>
          <span className="text-primary font-black">{MOVEMENT_DATA.id}</span>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start w-full h-full min-h-0 pb-32 lg:pb-0">
        <div className="lg:hidden w-full flex flex-col gap-3">
          <ModernSelect label="نوع الحركة" defaultValue={MOVEMENT_DATA.type} options={MOVEMENT_TYPES} />
          <ModernSelect label="المستودع" defaultValue={MOVEMENT_DATA.warehouse} options={['المستودع الرئيسي', 'مستودع طرابلس', 'مستودع بنغازي']} />
        </div>
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-6 w-full">
          <SpatialCard title="" hideHeader transparent>
            <div className="flex flex-col gap-4">
              {items.map((item, i) => (
                <ProductRowShell key={item.id} title={`الصنف ${i + 1}`} onRemove={() => removeItem(item.id)}>
                  <ModernSelect
                    label="اسم الصنف"
                    defaultValue={item.name}
                    options={PRODUCTS.filter((p) => p === item.name || !selectedNames.includes(p)).map((p) => ({ label: p }))}
                    onSelect={(name) => updateItem(item.id, { name })}
                  />
              ...
              <button onClick={addItem} disabled={items.filter(x => x.name).length >= PRODUCTS.length}
                ...
                <Plus className="w-5 h-5" />إضافة سطر صنف جديد
              </button>
            </div>
          </SpatialCard>
        </div>
        ...
        <aside className="w-full lg:w-[400px] xl:w-[440px] shrink-0 flex flex-col gap-5 lg:sticky lg:top-0 lg:self-start">
          ...
          <SpatialCard title="ملاحظات" icon={<FileText className="w-5 h-5" />}>
            <textarea defaultValue={MOVEMENT_DATA.notes} className="spatial-input w-full min-h-[80px] resize-none rounded-[20px] p-5 text-sm font-bold" placeholder="أضف أي ملاحظات..." />
          </SpatialCard>
          ...
          <div className="flex flex-col gap-4 max-lg:fixed max-lg:bottom-0 max-lg:left-0 max-lg:right-0 max-lg:z-[100] max-lg:bg-white/95 max-lg:dark:bg-slate-900/95 max-lg:px-5 max-lg:py-4 max-lg:backdrop-blur-xl max-lg:rounded-t-[24px] max-lg:border-t max-lg:border-black/10 max-lg:dark:border-white/10">
            <div className="lg:hidden flex items-center justify-between gap-3">
              <div className="flex flex-col gap-0.5">
                ...
                <div className="flex items-baseline gap-1.5">
                  <span className="text-2xl font-black text-primary">{totalQty}</span>
                  <span className="text-xs font-bold text-slate-400 dark:text-white/40">قطعة</span>
                </div>
              </div>
            ...
            <SpatialCard title="ملخص الحركة" className="hidden lg:block">
              <div className="flex flex-col gap-2">
                {items.filter(x => x.name).map((x) => (
                  <div key={x.id} className="flex items-center justify-between py-2 border-b border-black/5 dark:border-white/5 last:border-0">
                    ...
                    <span className="text-[14px] font-black text-slate-700 dark:text-white/80">{x.qty} قطعة</span>
                  </div>
                ))}
                <div className="flex items-center justify-between pt-3 border-t border-black/5 dark:border-white/5 mt-1">
                  <span className="text-[15px] font-black text-slate-700 dark:text-white/80">إجمالي الكميات</span>
                  <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-black text-primary">{totalQty}</span>
                    <span className="text-base font-bold text-slate-400 dark:text-white/40">قطعة</span>
                  </div>
                </div>
              </div>
            </SpatialCard>
