...
import { ArrowRight, Printer, CheckCircle2, Ban, Pencil, ArrowDownCircle, ArrowUpCircle, ArrowLeftRight, Clock, XCircle } from 'lucide-react';
import { SpatialCard } from '../compenntes/ui/SpatialComponents';
import { UploadArea } from '../compenntes/ui/UploadArea';

type Status = 'قيد الانتظار' | 'مكتمل' | 'ملغي';
...
const MOVEMENTS = {
  pending: {
    id: '#WM-20260408-00007', date: '2026-04-08', time: '09:00 AM', status: 'قيد الانتظار' as Status,
    warehouse: 'المستودع الرئيسي', type: 'إدخال',
    products: [
      { name: 'سحاب نايلون',   qty: 200 },
      { name: 'بطانة حريرية',  qty: 60  },
    ],
    log: [
      { label: 'تم إنشاء الحركة', by: 'أحمد المدير', date: '2026-04-08 09:00 AM' },
    ],
  },
  completed: {
...
const DEMO_KEYS = ['pending', 'completed', 'cancelled'] as const;
const DEMO_LABELS = { pending: 'قيد الانتظار', completed: 'مكتمل', cancelled: 'ملغي' };

export default function WarehouseMovementDetailPage() {
  const { id } = useParams();
  const navigate = useNavigate();
  const decodedId = id ? decodeURIComponent(id) : '';
  const initKey = decodedId === MOVEMENTS.completed.id ? 'completed' : decodedId === MOVEMENTS.cancelled.id ? 'cancelled' : 'pending';
      ...
      <div className="flex flex-col gap-6 overflow-y-auto custom-scroll pb-32 lg:pb-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:items-start w-full">
          ...
          <div className="w-full lg:w-[340px] xl:w-[380px] shrink-0 flex flex-col gap-5">
            ...
            {mov.status === 'قيد الانتظار' && (
              <SpatialCard title="صورة الحركة المختومة">
                <UploadArea />
                <div className="flex flex-col gap-2 mt-4">
                  <button className="spatial-button w-full h-11 rounded-[16px] flex items-center justify-center gap-2 font-bold text-[14px]">
                    <CheckCircle2 className="w-4 h-4" />تأكيد الحركة
                  </button>
                  <button className="w-full h-11 rounded-[16px] flex items-center justify-center gap-2 font-bold text-[14px] bg-red-500/10 hover:bg-red-500 border border-red-500/30 hover:border-red-500 text-red-500 hover:text-white transition-all">
                    <XCircle className="w-4 h-4" />إلغاء الحركة
                  </button>
                </div>
              </SpatialCard>
            )}
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-md animate-in fade-in duration-200" onClick={() => setShowImageModal(false)}>
          <div className="spatial-card p-5 w-[90vw] max-w-lg flex flex-col gap-4 animate-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <span className="text-[15px] font-black text-slate-800 dark:text-white">صورة الحركة المختومة</span>
