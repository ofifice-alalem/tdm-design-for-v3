...
// بيانات الفاتورة المراد تعديلها
const INVOICE_DATA = {
  id: '#SI-20260406-00153',
  store: 'المتجر الرئيسي - طرابلس',
  notes: 'تسليم عاجل',
  items: [
    { id: 1, name: 'سماعات ايربودز', price: 320, qty: 5, gift: 0, giftRule: { buyQty: 5, freeQty: 1 } },
    { id: 2, name: 'كابل الشحن السريع', price: 25, qty: 10, gift: 0, giftRule: { buyQty: 10, freeQty: 1 } },
  ],
  discountInvoice: 0,
};
...
export default function EditInvoicePage() {
  ...
  const [items, setItems] = useState<ProductItem[]>(INVOICE_DATA.items);
  const [discountInvoice, setDiscountInvoice] = useState(INVOICE_DATA.discountInvoice);
      ...
      <div className="flex flex-col gap-6 h-full">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate(-1)} className="flex items-center gap-2 px-4 h-11 rounded-[16px] bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 border border-black/10 dark:border-white/10 text-slate-600 dark:text-white/70 hover:text-slate-900 dark:hover:text-white transition-all font-bold text-[14px]">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-400 dark:text-white/30 font-bold">
              <span>الفواتير</span>
              <span>/</span>
              <span className="text-slate-700 dark:text-white/80">تعديل فاتورة</span>
              <span>/</span>
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start w-full h-full min-h-0 pb-32 lg:pb-0">
          ...
          <div className="lg:hidden w-full">
            <ModernSelect label="اختيار المتجر" defaultValue={INVOICE_DATA.store} options={['المتجر الرئيسي - طرابلس', 'فرع بنغازي', 'فرع مصراتة']} />
          </div>
          ...
          <aside className="w-full lg:w-[400px] xl:w-[440px] shrink-0 flex flex-col gap-5 lg:sticky lg:top-0 lg:self-start">
            ...
            <SpatialCard title="ملاحظات تفصيلية" icon={<FileText className="w-5 h-5" />}>
              <textarea
                defaultValue={INVOICE_DATA.notes}
...
function SummaryRow({ label, value, unit, isDiscount = false, editable = false, onEdit }: {
  ...
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-black/5 dark:border-white/5 last:border-0">
      ...
      <div className="flex items-baseline gap-1.5">
        {editable ? (
          <input type="number" defaultValue={0} onChange={(e) => onEdit?.(e.target.value)}
