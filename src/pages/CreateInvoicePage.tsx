...
interface ProductItem { id: number; name: string; price: number; qty: number; gift: number; }

export default function CreateInvoicePage() {
  const [items, setItems] = useState<ProductItem[]>([
    { id: 1, name: '', price: 0, qty: 0, gift: 0 },
    { id: 2, name: '', price: 0, qty: 0, gift: 0 },
  ]);
  const [discountProducts, setDiscountProducts] = useState(0);
  const [discountInvoice, setDiscountInvoice] = useState(0);

  const addItem = () => setItems((p) => [...p, { id: Date.now(), name: '', price: 0, qty: 0, gift: 0 }]);
  ...
  const totalPrice = useMemo(() => items.reduce((s, x) => s + x.price * x.qty, 0), [items]);
            ...
            ">
              ...
              <div className="hidden lg:block bg-black/5 dark:bg-white/5 backdrop-blur-xl rounded-[24px] border border-black/5 dark:border-white/5 p-7 transition-all">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <SummaryRow label="السعر الكلي" value={fmt(totalPrice)} unit="دينار" />
                  <SummaryRow label="تخفيض المنتجات" value={fmt(discountProducts)} unit="دينار" isDiscount
                    editable onEdit={(v) => setDiscountProducts(Number(v) || 0)} />
                  <SummaryRow label="تخفيض الفاتورة" value={fmt(discountInvoice)} unit="دينار" isDiscount
...
function ProductRow({ title, item, onRemove, onChange }: {
  ...
  function handleSelect(name: string) {
    ...
    if (found) onChange({ name: found.name, price: found.price });
  }
    ...
    <div className="bg-black/5 dark:bg-black/20 p-4 lg:p-6 rounded-[24px] border border-black/5 dark:border-white/5 flex flex-col gap-3 lg:gap-4 transition-colors duration-500">
      ...
      <div className="grid grid-cols-3 lg:grid-cols-4 gap-2 lg:gap-4">
        ...
        <ModernInput label="هدية" type="number" placeholder="0"
          value={item.gift > 0 ? String(item.gift) : ''}
          onChange={(v) => onChange({ gift: Number(v) || 0 })} />
